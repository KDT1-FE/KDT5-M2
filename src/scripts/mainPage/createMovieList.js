...
function createMovieList() {
  ...
  searchForm.addEventListener('submit', async event => {
    ...
    try {
      ...
      resultBox.addEventListener('scroll', () =>
        debouncedHandleScroll(resultBox)
      )
  ...
  async function fetchMovieList() {
    ...
    const title = $('.select:first-child').value
    const year = $('.select:nth-child(2)').value
    resultList.appendChild(loading)

    const moviesRes = await fetchMovies(inputText, year, title, searchPage)
    loading.remove()
